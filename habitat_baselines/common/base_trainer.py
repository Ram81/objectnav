...
class BaseRLTrainer(BaseTrainer):
    ...
    def eval(self, eval_ckpt=None, log_diagnostics=[], output_dir=".", label="eval", skip_log=False, simple_eval=False) -> None:
        ...
        with TensorboardWriter(
            ...
            if eval_ckpt is not None: # evaluate a single checkpoint from path
                ...
                self._eval_checkpoint(eval_ckpt, writer, checkpoint_index=ckpt_index, log_diagnostics=log_diagnostics, output_dir=output_dir, label=label, skip_log=skip_log, simple_eval=simple_eval)
            else:
    ...
    def _pause_envs(
        ...
        if len(envs_to_pause) > 0:
            ...
            for k, v in d_stats.items():
                d_stats[k] = [v[i] for i in state_index]
